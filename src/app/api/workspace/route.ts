  ...
  try {
    ...
    const [people, requests, settings] = await Promise.all([
      ...
      db
        .select()
        .from(tickets)
        .orderBy(desc(tickets.requestedAt), desc(tickets.id)),
    ...
    const normalizedRequests = requests.map((ticket) => ({
      ...ticket,
      initialAction:
        ticket.initialAction ||
        (ticket.status === "Resolved" ? "" : ticket.action),
      resolvedAction:
    ...
    return NextResponse.json({
      ...
      itPersonnelPosition:
        config?.itPersonnelPosition ?? defaults.itPersonnelPosition,
    });
  } catch (error) {
    console.error(error);
    return NextResponse.json(
      { error: "Unable to connect to the local database. Please try again." },
      { status: 500 },
    );
  }
}
  ...
  try {
    ...
    if (body.entity === "settings") {
      ...
      const values = {
        ...
        headerGovernment:
          clean(body.headerGovernment, 250) || defaults.headerGovernment,
        headerDepartment:
          clean(body.headerDepartment, 250) || defaults.headerDepartment,
        ...
        itPersonnelName:
          clean(body.itPersonnelName, 150) || defaults.itPersonnelName,
        itPersonnelPosition:
          clean(body.itPersonnelPosition, 150) || defaults.itPersonnelPosition,
      };
    ...
    if (body.entity === "employee") {
      const values = {
        ...
        office: clean(body.office, 200),
        position: clean(body.position, 200),
        email: clean(body.email, 200),
      };
      if (!values.name || !values.office || !values.position)
        return NextResponse.json(
          { error: "Name, office, and position are required." },
          { status: 400 },
        );
    ...
    if (body.entity === "ticket") {
      const [person] = await db
        .select()
        .from(employees)
        .where(eq(employees.id, Number(body.employeeId) || 0));
      if (!person)
        return NextResponse.json(
          { error: "Please select a registered employee." },
          { status: 400 },
        );
      const date = new Date(String(body.requestedAt ?? ""));
      if (
        !clean(body.issue, 200) ||
        !clean(body.description) ||
        isNaN(date.getTime())
      )
        return NextResponse.json(
          { error: "Request date, issue, and description are required." },
          { status: 400 },
        );
      ...
      const initialAction = clean(
        body.initialAction ?? (status === "Resolved" ? "" : legacyAction),
      );
      const resolvedAction =
        status === "Resolved"
          ? clean(
              body.resolvedAction ??
                (body.initialAction === undefined ? legacyAction : ""),
            )
          : "";
      if (status === "Resolved" && !resolvedAction)
        return NextResponse.json(
          {
            error: "Record the resolved action before resolving this request.",
          },
          { status: 400 },
        );
      ...
      if (body.id) {
        const [old] = await db
          .select()
          .from(tickets)
          .where(eq(tickets.id, Number(body.id)));
        if (!old)
          return NextResponse.json(
        ...
        if (old.status === "Resolved" && status === "Resolved")
          values.resolvedAt = old.resolvedAt;
        const [result] = await db
          .update(tickets)
          .set(values)
          .where(eq(tickets.id, old.id))
          .returning();
        return NextResponse.json(result);
      }
    ...
    if (body.entity === "clear-demo") {
      ...
      await db.transaction(async (tx) => {
        const [settings] = await tx
          ...
          .for("update");
        if (!settings?.demo)
          throw new Error("Sample workspace has already been cleared.");
    ...
    console.error(error);
    return NextResponse.json(
      { error: "Could not save your changes. Please refresh and try again." },
      { status: 500 },
    );
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    const entity = url.searchParams.get("entity") ?? "employee";
    if (!id)
      return NextResponse.json(
    ...
    const related = await db
      ...
      .from(tickets)
      .where(eq(tickets.employeeId, id))
      .limit(1);
    if (related.length)
      return NextResponse.json(
        {
          error:
            "This employee has ticket history and cannot be deleted. You can still edit their details.",
        },
        { status: 409 },
      );
    const deleted = await db
